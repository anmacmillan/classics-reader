...
IMPORTED_DANISH_DICTIONARY_PATH = ROOT / "generated" / "imported-danish-dictionary.js"
...
def load_dictionary_keys(dictionary_name: str) -> set[str]:
    standalone_paths = {
        ...
        "DANISH_DICT": IMPORTED_DANISH_DICTIONARY_PATH,
    }
...
def load_import(import_dir: Path) -> tuple[dict, list[list[str]]]:
    ...
    if manifest["lang"] not in {"greek", "latin", "old_english", "middle_dutch", "dutch", "danish"}:
        raise ValueError(f"{import_dir.name}: lang must be greek, latin, old_english, middle_dutch, dutch, or danish")
...
def main() -> int:
    ...
    dictionary_keys = {
        ...
        "danish": load_dictionary_keys("DANISH_DICT"),
    }
    books = []
    seen_ids = set()
