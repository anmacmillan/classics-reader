...
test('loads pagination policy before the versioned application and provides a hidden reading-mode button', () => {
  ...
  const appScript = '<script src="app.js?v=20260923-2"></script>';
...
test('debounces resize composition and retains the semantic anchor captured before reflow', () => {
  assert.match(app, /window\.addEventListener\(\s*["']resize["']\s*,\s*\(\)\s*=>\s*\{[\s\S]*?const\s+anchorLineIndex\s*=\s*captureReadingAnchor\(\);[\s\S]*?const\s+renderGeneration\s*=\s*readerRenderGeneration;[\s\S]*?clearTimeout\(resizeTimer\);[\s\S]*?resizeTimer\s*=\s*setTimeout\(\s*\(\)\s*=>\s*\{[\s\S]*?renderGeneration\s*!==\s*readerRenderGeneration[\s\S]*?recalcPages\(\{\s*anchorLineIndex\s*\}\)/);
});
...
test('wires touch and keyboard page turns through the pagination policy without overlay zones', () => {
  ...
  assert.match(app, /readerPane\.addEventListener\(\s*["']pointerdown["']\s*,\s*handleReaderPointerDown\s*\)/);
  assert.match(app, /readerPane\.addEventListener\(\s*["']pointercancel["']\s*,\s*handleReaderPointerCancel\s*\)/);
  ...
  assert.match(app, /function\s+handleReaderKeydown\s*\(\s*event\s*\)[\s\S]*?event\.altKey\s*\|\|\s*event\.ctrlKey\s*\|\|\s*event\.metaKey\s*\|\|\s*event\.shiftKey[\s\S]*?ReaderPagination\.isInteractiveTarget\(event\.target\)[\s\S]*?event\.key\s*===\s*["']ArrowLeft["'][\s\S]*?event\.preventDefault\(\)/);
  assert.match(app, /function\s+navigatePaged\s*\(\s*direction\s*\)[\s\S]*?ReaderPagination\.navigationDecision\([\s\S]*?pendingPageEdge\s*=\s*decision\.edge[\s\S]*?renderChapter\(\{\s*syncAfterPlacement:\s*true\s*\}\)/);
