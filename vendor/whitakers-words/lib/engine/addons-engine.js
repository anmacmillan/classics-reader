 ...
 */
export function tryTackons(word, tackons, packons, inflIndex, dictIndex) {
    const results = [];
    ...
    for (const packon of packons) {
        ...
        if (word.length > tack.length && word.endsWith(tack)) {
            ...
            const baseResults = getBase(base).filter((r) => r.ir.qual.pofs === "PACK" || r.ir.qual.pofs === "PRON");
 ...
 */
export function tryPrefixes(word, prefixes, inflIndex, dictIndex) {
    const results = [];
    for (const prefix of prefixes) {
        const fix = prefix.fix.toLowerCase();
        if (word.length <= fix.length)
            continue;
 ...
 */
export function trySuffixes(word, suffixTrie, inflIndex, dictIndex) {
    const results = [];
        ...
        for (const { suffix, strippedStem } of matches) {
            ...
            for (const ds of dictStems) {
                ...
                if (!entry)
                    continue;
                // Filter by root POS
                if (suffix.entr.root !== "X" && entry.part.pofs !== suffix.entr.root)
                    continue;
                // Filter by root key (0 = match any)
                if (suffix.entr.rootKey !== 0 && suffix.entr.rootKey !== ds.stemKey)
                    continue;
                ...
                for (const pair of stemPairs) {
                    if (targetPofs !== "X" && pair.ir.qual.pofs !== targetPofs)
                        continue;
                    if (targetKey !== 0 && pair.ir.key !== targetKey)
                        continue;
 ...
 */
export function tryAddons(word, addons, suffixTrie, inflIndex, dictIndex) {
    const results = [];
