...
export function isSumForm(word) {
    if (word.length === 0)
        return null;
    const first = word[0];
    // Fast reject: sum forms only start with s, e, f
    if (first !== "s" && first !== "e" && first !== "f")
        return null;
 ...
 */
function getCompoundTense(sumTense) {
    switch (sumTense) {
        case "PRES":
        case "PERF":
            return "PERF";
 ...
 */
export function tryCompound(results, nextWord) {
