 ...
 */
export function applyTrick(word, trick) {
    const results = [];
    switch (trick.op) {
        case "flip": {
 ...
 */
export function getTricksForWord(word) {
    if (word.length === 0)
        return [];
    const first = word[0];
    if (first === undefined)
    ...
    const specific = charTricks ?? [];
    return [...specific, ...ANY_TRICKS];
}
/**
 * Return all medieval trick rules.
 */
export function getMedievalTricks() {
 ...
 */
export function applyTricks(word) {
 ...
 */
export function trickAnnotation(trick) {
