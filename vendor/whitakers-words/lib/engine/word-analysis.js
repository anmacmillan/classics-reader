 ...
 */
export function runInflections(word, inflectionIndex) {
    const results = [];
 ...
 */
export function searchDictionaries(pairs, dictIndex) {
    const results = [];
 ...
 */
function matchAndResolve(ir, de, stemKey) {
    const qual = ir.qual;
    const part = de.part;
...
function matchAndResolveQuality(qual, part, stemKey) {
    switch (qual.pofs) {
        case "N":
            if (part.pofs !== "N")
                return null;
            if (!matchesDecn(qual.noun.decl, part.n.decl))
                return null;
            // Ada: Pdl_Part.N.Gender <= Sl(I).IR.Qual.Noun.Gender
            // Dictionary gender must be contained in inflection gender
            if (!matchesGender(part.n.gender, qual.noun.gender))
                return null;
        ...
        case "PRON":
            if (part.pofs !== "PRON")
                return null;
            if (!matchesDecn(qual.pron.decl, part.pron.decl))
                return null;
            return {
                pofs: "PRON",
                pron: {
        ...
        case "ADJ": {
            if (part.pofs !== "ADJ")
                return null;
            if (!matchesDecn(qual.adj.decl, part.adj.decl))
                return null;
        ...
        case "NUM":
            if (part.pofs !== "NUM")
                return null;
            if (!matchesDecn(qual.num.decl, part.num.decl))
                return null;
            return {
                pofs: "NUM",
                num: {
        ...
        case "ADV": {
            ...
            if (co !== qual.adv.comparison && qual.adv.comparison !== "X")
                return null;
        ...
        case "V":
            if (part.pofs !== "V")
                return null;
            if (!matchesDecn(qual.verb.con, part.v.con))
                return null;
            return {
                pofs: "V",
                verb: {
        ...
        case "VPAR":
            if (part.pofs !== "V")
                return null;
            if (!matchesDecn(qual.vpar.con, part.v.con))
                return null;
            return {
                pofs: "VPAR",
                vpar: {
        ...
        case "SUPINE":
            if (part.pofs !== "V")
                return null;
            if (!matchesDecn(qual.supine.con, part.v.con))
                return null;
            return {
                pofs: "SUPINE",
                supine: {
 ...
 */
function matchesDecn(inflDecn, dictDecn) {
    // Exact match
...
function adjCompFromKey(key) {
    return ADJ_COMP_FROM_KEY[key] ?? "X";
}
/** Ada's Adv_Comp_From_Key: stem key → adverb comparison level. */
const ADV_COMP_FROM_KEY = { 1: "POS", 2: "COMP", 3: "SUPER" };
function advCompFromKey(key) {
 ...
 */
export function analyzeWord(word, inflectionIndex, dictIndex) {
