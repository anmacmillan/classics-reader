 ...
 */
export function listSweep(results) {
...
export function filterByPOS(results) {
    return results.filter((r) => {
        ...
        if (kind === "DEP") {
            if (tvm.voice === "ACTIVE" && tvm.tense === "FUT" && tvm.mood === "INF")
                return true;
            if (tvm.voice === "ACTIVE" &&
 ...
 */
export function normalizeDisplay(results) {
    return results.map(normalizeDisplayValues);
}
function normalizeDisplayValues(r) {
    const q = r.ir.qual;
