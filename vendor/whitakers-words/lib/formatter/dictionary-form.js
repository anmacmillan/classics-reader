 ...
 */
export function dictionaryForm(de) {
    const part = de.part;
    const s = de.stems;
    switch (part.pofs) {
        case "N":
...
function ordinal(which) {
    return ORDINALS[which] ?? "";
}
function formatNoun(stem1, stem2, which, variant, gender) {
    let nom;
    let gen;
    switch (which) {
        case 1:
 ...
 */
function formatPronoun(stem1, stem2, which, variant) {
    let ox1 = null;
    let ox2 = null;
    let ox3 = null;
    switch (which) {
...
function formatVerb(stem1, stem2, stem3, stem4, which, variant, kind) {
    const parts = [];
    // --- Deponent verbs: passive forms (table-driven) ---
    if (kind === "DEP") {
        const DEP_OX1 = { 1: "or", 2: "eor", 3: "or" };
        const DEP_INF = { "1": "ari", "2": "eri", "3": "i", "3.4": "iri" };
        parts.push(`${stem1}${DEP_OX1[which] ?? "or"}`);
        parts.push(`${stem2 || stem1}${DEP_INF[`${which}.${variant}`] ?? DEP_INF[String(which)] ?? "i"}`);
    ...
    if (kind === "IMPERS") {
        const IMPERS_OX1 = { 1: "at", 2: "et", 3: "it" };
        if (which === 5 && variant === 1)
            parts.push(`${stem1}est`);
        else
            parts.push(`${stem1}${IMPERS_OX1[which] ?? "t"}`);
    }
    else {
        const OX1 = { "2": "eo", "5": "um", "7.2": "am" };
    ...
    if (kind === "IMPERS") {
        if (hasStem(stem3))
            parts.push(`${stem3}it`);
        else if (hasStem(stem4))
            parts.push("-");
        if (hasStem(stem4))
            parts.push(`${stem4}us est`);
        else if (hasStem(stem3))
            parts.push("-");
    }
    else if (kind === "SEMIDEP") {
        // Ada: only Ox(4) is set for SEMIDEP — stem3 (active perfect) is skipped
        if (hasStem(stem4))
            parts.push(`${stem4}us sum`);
    }
...
function formatVerbResult(parts, which, variant, kind) {
...
function formatAdjective(stems, which, variant, co) {
    const [s1, s2, s3, s4] = stems;
    // Comparative only
    if (co === "COMP")
        return `${s1}or, ${s1}or, ${s1}us  ADJ`;
    // Superlative only
    if (co === "SUPER")
        return `${s1}mus, ${s1}ma, ${s1}mum  ADJ`;
    ...
    if (which === 3) {
        if (variant === 1)
            return `${s1}, (gen.), ${s2 || s1}is  ADJ`;
        if (variant === 2)
            return `${s1}is, ${s2 || s1}is, ${s2 || s1}e  ADJ`;
        if (variant === 3)
            return `${s1}, ${s2 || s1}is, ${s2 || s1}e  ADJ`;
        if (variant === 6)
            return `${s1}, (gen.), ${s2 || s1}os  ADJ`;
    }
    // 1st/2nd declension positive
    if (variant === 2 || variant === 4)
        return `${s1}, ${s2 || s1}a, ${s2 || s1}um  ADJ`;
    if (variant === 3)
        return `${s1}us, ${s2 || s1}a, ${s2 || s1}um (gen -ius)  ADJ`;
