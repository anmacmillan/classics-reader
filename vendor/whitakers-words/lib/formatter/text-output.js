import { extractQuality } from "../types/quality-extract.js";
import { dictionaryForm } from "./dictionary-form.js";
import { AGE_TAG_COLUMN, CASE_WIDTH, DECL_WIDTH, f, formatFields, GENDER_WIDTH, MAX_MEANING_SIZE, MOOD_WIDTH, NUMBER_WIDTH, PERSON_WIDTH, POS_WIDTH, STEM_DOT_WIDTH, TENSE_WIDTH, TERMINAL_WIDTH, VOICE_WIDTH, } from "./field-layout.js";
/**
 ...
 */
export function formatWordAnalysis(analysis) {
    const lines = [];
    ...
    const compoundsByEntry = new Map();
    ...
    for (const block of rendered) {
        if (compoundsByEntry.size > 0) {
            ...
            if (dictFormIdx >= 0) {
                ...
                const groupResults = merged.find((g) => g.results.some((r) => compoundsByEntry.has(r.entryIndex)));
 ...
 */
export function groupAndMerge(results) {
 ...
 */
function mergeByMeaning(groups) {
...
function formatCompoundLine(cr) {
...
export function formatInflectionLine(r) {
...
function formatQualityLine(stem, ending, qual) {
 ...
 */
function formatQualityRecord(qual, verbKind) {
    const v = extractQuality(qual);
    return formatQualityFromValues(v, verbKind);
}
function formatQualityFromValues(v, verbKind) {
    ...
    if (v.pofs === "N" || v.pofs === "PRON" || v.pofs === "PACK" || v.pofs === "SUPINE") {
        ...
        if (v.decl)
            fields.push(f(v.decl[0], DECL_WIDTH), f(v.decl[1], DECL_WIDTH));
        if (v.cs)
    ...
    if (v.pofs === "ADJ" || v.pofs === "NUM") {
        ...
        fields.push(f(v.cs, CASE_WIDTH), f(v.number, NUMBER_WIDTH), f(v.gender, GENDER_WIDTH));
        if (v.comparison)
            fields.push(f(v.comparison, TERMINAL_WIDTH));
        if (v.sort)
            fields.push(f(v.sort, TERMINAL_WIDTH));
        return formatFields(fields).trimEnd();
    }
    // ADV: simple
    if (v.pofs === "ADV") {
